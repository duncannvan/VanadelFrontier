...
func handle_craft_button_press(can_craft: bool) -> void:
	if not can_craft:
		return
	...
	for slot: TextureButton in _slots:
		if slot.is_pressed():
			...
			get_parent().get_parent().add_child(tower_inst) # TODO: Find better way to add scene to main
			await get_tree().create_timer(CRAFT_BUTTON_DELAY).timeout
			placing_tower = true
			visible = false


func update_craft_recipe_ui(num_resources: int) -> void:
