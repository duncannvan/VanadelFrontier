...
func _physics_process(delta: float) -> void:
	...
	var new_velocity: Vector2 = global_position.direction_to(next_path_position) * _stats_component.get_component(COMPONENT_KEY.SPEED).get_current_speed()
...
func get_health() -> int: 
	return _stats_component.get_component(COMPONENT_KEY.HEALTH).get_health()
